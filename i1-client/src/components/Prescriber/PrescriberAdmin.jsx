...
import {listPrescriberInfos, getPatientHcpProfiles} from '../../graphql/queries'
import {addUpdateHcpProfile} from '../../graphql/mutations'
...
const PrescriberAdmin = (props) => {
	...
	const prescriberInfoForm = {
		...
		npiNumber: {
			...
			validations: [
				// {
				//     type: "required",
				//     message: Constants.ErrorMessage.npiNo_REQUIRED,
				// },
			],
		},
		offAddr: {
	...
	const addUpdateHCPProfileCall = async (requestObject) => {
		try {
			...
			console.log(data)
			if (data && data.data && data.data.addUpdateHCPProfile) {
				setDialogOption({
					title: 'HCP Prescriber Info',
					message: 'HCP Profile saved sucessfully',
					showDialog: true,
				})
			}
	...
	const handleSubmit = (dataItem) => {
		...
		console.log("marty HCPAdmin form dataItem", dataItem)
		...
		const requestObject = {
			...
			hcpProfile: {
				...
				officeEmail: dataItem.officeEmail,
				officePhoneNumber: dataItem.officePhone,
				officeFaxNumber: dataItem.officeFax,
				...
				preferredPrescriberContactMethod: dataItem.preferredPrescriberContactMethod,
				notes: notesTableData.filter(item => item.inEdit).map(item => (
					{
						date: item.date,
						note: item.note
					}
		...
		console.log('marty HCPAdmin mutation requestObject', requestObject)
	...
	return (
		<div className="row">
						...
						<form onSubmit={formRenderProps.onSubmit} className={'k-form pl-3 pr-3 pt-1'}>
							<div className="row">
								<div className="col-md-3">
									<h3 className="pageTitle">Prescriber Information</h3>
								</div>
							</div>
							...
							<div className="row">
								...
								*/}
								<div className="col-md-3">
									<Field name={'prescName'} label={'HCP Name'} component={InputField} 
										/>
								</div>
							...
							<form onSubmit={formRenderProps.onSubmit} className={'k-form pl-3 pr-3 pt-1'}>
								...
								<div className="row mt-12">
									<div className="col-md-5">
										<Field name={'medSpec'} label={'Medical Specialty'} component={InputField}
											validator={prescriberInfoForm.medSpec.inputValidator} />
									</div>
									<div className="col-md-4"> 
										{/* need to update validations */}
										<Field name={'siteInstName'} label={'Practice Name'} component={InputField}
								...
								<div className="row">
									<div className="col-md-2">
										<Field name={'officePhone'} label={'Office Phone'} component={MaskedPhoneInput}
											validator={prescriberInfoForm.officePhone.inputValidator} />
									</div>
									<div className="col-md-2">
										<Field name={'officeFax'} label={'Office Fax'} component={MaskedPhoneInput}
											validator={prescriberInfoForm.officeFax.inputValidator} />
									</div>
									<div className="col-md-3">
										<Field name={'officeEmail'} label={'Office Email'} component={Input} />
									</div>
									<div className="col-md-2">
										<Field name={'hinNumber'} label={'HIN'} component={Input} />
									</div>
								</div>
			...
			{
				visibleDialog && (
					<Dialog title={'Prescriber SELECTION MODAL'} width={1000} onClose={togglePatientSearchDialog}>
						<Grid
...
export default PrescriberAdmin